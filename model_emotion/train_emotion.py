...
from pytorch_transformers import (GPT2Tokenizer, GPT2Model, AdamW, GPT2DoubleHeadsModel, WEIGHTS_NAME, CONFIG_NAME, OpenAIGPTDoubleHeadsModel, OpenAIGPTTokenizer)

from utils import get_dataset

#SPECIAL_TOKENS_dic = {"unk_token":"unk_token", "pad_token":"pad_token"}
SPECIAL_TOKENS = ["bos_token","eos_token","<speaker1>", "<speaker2>","pad_token"]
...
def build_input_from_segments( emotion, history, reply, tokenizer, lm_labels=False, with_eos=True):
	""" Build a sequence of input from 3 segments: persona, history and last reply """
	bos, eos, speaker1, speaker2 = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[:-1])

	instance = {}
	sequence = [emotion[-1]] +history + [reply + ([eos] if with_eos else [])]
	sequence =  [[speaker2 if (len(sequence)-i) % 2 else speaker1] + s for i, s in enumerate(sequence[1:])]
	#sequence =  [[speaker1 if (len(sequence)-i) % 2 else speaker2] + s for i, s in enumerate(sequence[:-1])] + [[speaker2] + sequence[-1]]

	instance["input_ids"] = list(chain(*sequence))
	instance["token_type_ids"] = [speaker1 if i % 2 else speaker2 for i, s in enumerate(sequence) for _ in s]
	#instance["token_type_ids"] = [speaker2 if s[0]==speaker2 else speaker1 for i, s in enumerate(sequence) for _ in s]
...
def get_data_loaders(args, tokenizer):
	...
	test_loader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.valid_batch_size, shuffle=False)
...
def train():
	...
	model_class = GPT2DoubleHeadsModel if "gpt2" in args.model_checkpoint else OpenAIGPTModel
	...
	train_loader, val_loader, test_loader, train_sampler, valid_sampler, test_sampler = get_data_loaders(args, tokenizer)

	# Training function and trainer
	def update(engine, batch):
		model.train()
		batch = tuple(input_tensor.to(args.device) for input_tensor in batch)
		lmmc_loss = model(*batch)
		lm_loss = lmmc_loss[0]
		mc_loss = lmmc_loss[1]
